...
import { firebaseConfig, appId } from "../../service/firebase";
import { parseInvoiceWithGemini, callGeminiAPI } from "../../service/gemini";
  ...
  useEffect(() => {
    if (user && db) {
      ...
      const budgetsQuery = query(
        collection(db, `artifacts/${appId}/users/${user.uid}/budgets`)
      );
      const goalsQuery = query(
        collection(db, `artifacts/${appId}/users/${user.uid}/goals`)
      );
      const alertsQuery = query(
        collection(db, `artifacts/${appId}/users/${user.uid}/alerts`)
      );
        ...
        {
          {
            Dashboard: (
              <DashboardScreen
                ...
                getDashboardInsights={() => getDashboardInsights(invoices)}
...
const DashboardScreen = ({
  ...
  return (
    <div className="space-y-6 h-full overflow-y-auto p-4 md:p-6 pb-24">
      ...
      <div className="bg-white/50 backdrop-blur-lg border border-white/20 p-5 rounded-2xl shadow-xl">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-lg font-semibold flex items-center text-black">
            <Sparkles
              size={20}
              className="mr-2 text-black drop-shadow-[0_0_5px_rgba(0,0,0,0.5)] animate-pulse"
            />
            Smart Insights
          </h2>
          <motion.button
          ...
          >
            <RefreshCw
              size={16}
              className={isInsightsLoading ? "animate-spin" : ""}
            />
          </motion.button>
        </div>
        <div className="text-sm text-gray-700 space-y-3 leading-relaxed">
          {isInsightsLoading ? (
            ...
            <p className="text-gray-500">
              Scan more invoices to unlock personalized insights!
            </p>
          )}
        </div>
      </div>
          ...
          {alerts.filter((a) => !a.paid).length > 0 ? (
            alerts
              ...
              .map((alert) => (
                <div
                  key={alert.id}
                  className="flex justify-between items-center p-3 bg-white/30 rounded-lg"
                >
                  <div>
                    <p className="font-semibold">{alert.name}</p>
                    <p className="text-sm text-gray-600">
                      {new Date(
                        alert.dueDate.seconds * 1000
                      ).toLocaleDateString()}
                    </p>
                  </div>
                  <p className="font-bold text-lg text-black">
                    {getCurrencySymbol(alert.currency)}
                ...
                <div key={goal.id}>
                  <div className="flex justify-between items-center mb-1 text-sm">
                    ...
                    <span className="font-semibold text-gray-500">
                      {getCurrencySymbol(goal.currency || "USD")}
...
const InvoicesScreen = ({ invoices, db, userId, setNotification }) => {
  ...
  const filteredInvoices = invoices
    .filter(
      (invoice) =>
        (invoice.vendorName?.toLowerCase() || "").includes(
          searchTerm.toLowerCase()
        ) ||
        (invoice.category?.toLowerCase() || "").includes(
          searchTerm.toLowerCase()
        )
    )
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      await deleteDoc(
        doc(db, `artifacts/${appId}/users/${userId}/invoices`, invoiceToDelete)
      );
      setNotification({
          ...
          >
            ...
            >
              ...
              <div className="flex items-center space-x-4">
                ...
                <motion.div
                  animate={{
                    rotate: expandedInvoiceId === invoice.id ? 180 : 0,
                  }}
                >
                  <ChevronDown />
                </motion.div>
                  ...
                  <div className="px-4 pb-4 border-t border-white/20">
                    ...
                    <div className="space-y-1 text-sm text-gray-600">
                      {invoice.lineItems && invoice.lineItems.length > 0 ? (
                        ...
                        <p className="text-xs">
                          No detailed items were extracted for this invoice.
                        </p>
                      )}
                    </div>
                  </div>
...
const ScanScreen = ({ db, userId, setActiveScreen, setNotification }) => {
  ...
  const processImageWithAI = async (file) => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        const structuredData = await parseInvoiceWithGemini(
          base64ImageData,
          setNotification
        );
        if (structuredData) {
          ...
          setNotification({
            text: "Invoice details extracted!",
            type: "success",
          });
        } else {
          setNotification({
            text: "AI couldn't extract details. Please enter manually.",
            type: "error",
          });
          setInvoiceData({
            vendorName: "",
  ...
  const handleSaveInvoice = async () => {
    if (!invoiceData.vendorName || !invoiceData.totalAmount) {
      setNotification({
        text: "Vendor Name and Total Amount are required.",
        type: "error",
      });
      return;
    }
    if (!db || !userId) {
      setNotification({
        text: "Database not ready. Cannot save.",
        type: "error",
      });
      return;
    }
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setInvoiceData({ ...invoiceData, vendorName: e.target.value })
              }
...
const ChatScreen = ({
  ...
  useEffect(() => {
    ...
    const messagesQuery = query(
      collection(
        db,
        `artifacts/${appId}/users/${userId}/chats/${activeChatSessionId}/messages`
      ),
      orderBy("createdAt")
    );
  ...
  const handleDeleteChat = async () => {
    ...
    const messagesQuery = query(
      collection(
        db,
        `artifacts/${appId}/users/${userId}/chats/${chatToDelete}/messages`
      )
    );
            ...
            {messages.length === 0 && !isLoading && (
              <div className="text-center text-gray-500 pt-10">
                ...
                <p className="text-xs mt-2">
                  e.g., "How much did I spend on shopping this month?"
                </p>
              </div>
            )}
            <AnimatePresence>
...
const BudgetScreen = ({ db, userId, budgets, setNotification }) => {
  ...
  const handleAddBudget = async (e) => {
    ...
    try {
      ...
      setNotification({
        text: `Failed to add budget: ${error.message}`,
        type: "error",
      });
    } finally {
      setIsSaving(false);
  ...
  const handleDeleteBudget = async (budgetId) => {
    try {
      await deleteDoc(
        doc(db, `artifacts/${appId}/users/${userId}/budgets`, budgetId)
      );
...
const GoalsScreen = ({ db, userId, goals, setNotification }) => {
  ...
  const handleAddGoal = async (e) => {
    ...
    if (!name || !targetAmount || isNaN(parseFloat(targetAmount))) {
      setNotification({
        text: "Please enter a valid name and target amount.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      setNotification({
        text: `Failed to add goal: ${error.message}`,
        type: "error",
      });
    } finally {
      setIsSaving(false);
...
const AlertsScreen = ({ db, userId, alerts, setNotification }) => {
  ...
  const handleAddAlert = async (e) => {
    ...
    try {
      await addDoc(
        collection(db, `artifacts/${appId}/users/${userId}/alerts`),
        {
          name,
          amount: parseFloat(amount),
      ...
      setNotification({
        text: `Failed to add alert: ${error.message}`,
        type: "error",
      });
    } finally {
      setIsSaving(false);
  ...
  const handleDeleteAlert = async (alertId) => {
    try {
      await deleteDoc(
        doc(db, `artifacts/${appId}/users/${userId}/alerts`, alertId)
      );
          ...
          {alerts.length > 0 ? (
            alerts.map((alert) => (
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  <p className="font-bold text-lg text-black">
                    {getCurrencySymbol(alert.currency || "USD")}
                    {alert.amount.toFixed(2)}
                  </p>
                  <motion.button
                    whileHover={{ scale: 1.1 }}
      ...
      <div className="flex justify-around items-center max-w-lg mx-auto h-20">
        {leftItems.map((item) => {
          ...
          return (
            <div
              key={item.name}
              className="w-20 relative flex justify-center items-center h-full"
            >
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={() => setActiveScreen(item.name)}
                className={`relative z-10 flex flex-col items-center justify-center w-full h-full transition-colors duration-200 ${
